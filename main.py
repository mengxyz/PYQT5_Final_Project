...
class Mainwindow(QMainWindow):
    ...
    def save_click(self):
        ...
        if self.save_stat == 'add' :
            sql = "insert into Book(name,cate) values('"+ str(self.ui.lineEdit.text()) +"','"+ str(self.ui.comboBox.currentData()) +"')"
        elif self.save_stat == 'edit' :
            sql = "update Book set name = '"+ str(self.ui.lineEdit.text()) +"',cate = '"+ str(self.ui.comboBox.currentData()) +"' where id = "
            sql += "'"+ self.ui.tableWidget.item(row,0).text() +"'"
        else:
            msg = QMessageBox.question(
                self,'ยืนยัน','ต้องการลบใช่หรือไม่',
                QMessageBox.Yes|QMessageBox.No,
                QMessageBox.No
            )
            if(msg == QMessageBox.Yes):
                sql = "delete from Book where id = '"+ self.ui.tableWidget.item(row,0).text() +"'"
            else:
                return
        ...
        if(
            ...
            msg = QMessageBox()
            msg.critical(
                self,'error','กรุณาใส่ข้อมูลให้ครบ',
                QMessageBox.Ok,
                QMessageBox.Ok
            )
        else:
    ...
    def twg_Load(self):
        sql = "select b.id,b.name,c.name from Book b left join Cate c on b.cate = c.id"
